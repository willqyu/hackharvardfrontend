...
const GradientGauge = ({ value }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center my-10">
      <div className="relative w-72 h-8 rounded-lg overflow-hidden">
        {/* Gradient background */}
        <div className="absolute inset-0 bg-gradient-to-r from-red-500 to-green-500"></div>
        {/* Needle */}
        <div
          ref={needleRef} // Attach the ref to the needle
          className="absolute w-1 h-8 bg-black transition-left duration-300 ease-in-out" // Add transition for left property
          style={{
            transform: 'translateX(-50%)', // Center the needle on its position
          }}
        />
      </div>
      <div className="mt-2 text-xl font-bold">{clampedValue.toFixed(1)}</div>
    </div>
  );
};
